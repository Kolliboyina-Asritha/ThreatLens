/**
 * ThreatLens AI — Dual-Tier Explanation Registry for Browser Protection
 */
export const DUAL_TIER_DATABASE = {
  ...
  RANDOM_FOREST_ML: {
    ...
    technical: 'An ensemble of 100 decision trees evaluating 18 lexical and structural URL dimensions trained on real Tranco and PhishTank/URLhaus feeds.',
...
export const getExplanationForIndicator = (code) => {
  const key = String(code || '').toUpperCase();
  return DUAL_TIER_DATABASE[key] || {
    title: code?.replace(/_/g, ' ') || 'Security Anomaly',
    badge: 'Security Finding',
    technical: `Forensic indicator "${code}" flagged by ThreatLens security engines.`,
    simple: `ThreatLens detected an unusual pattern with this web link (${code}).`,
    whyItMatters: 'Unusual address patterns frequently correlate with phishing, spoofing, or malicious redirection.'
  };
};
